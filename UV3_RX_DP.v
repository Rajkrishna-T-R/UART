        ...
        module UV3_RX_DP(
                        ...
                        input wire inc_BiCount,   // increment signal for bit index while sampling the data 
                        ...
                        input wire sample_data_en,      // Only when this signal is enabled the bit is entered to the register
           ...
           assign DS_count=samp_count;         // sampling count for the Rx controller 
           assign BI_count=bit_index_count;    // bit index count for the Rx controller 
        ...
        // Synchronising 
         ...
         //--------------------------------------
          ...
          always@(posedge clk or posedge RST)
            ...
            assign start_bit_edge=(((Rx_prev)&&(~Rx_in)));  
            // detect the start bit using the values in the flip flops
         ...
         //---------------------------------------
            ...
            always @(posedge clk or posedge RST)
                begin
                     if(RST==1'b1)
                        begin  
                        // Reset the counts and serial data register 
                            samp_count       <= 0;
                            bit_index_count  <= 0;
                            S_Data_reg       <= 0; 
                        end
                        ...
                        begin
                            ...
                            else if(tick==1 && clear_data==1'b0 && sample_data_en)
                                begin
        
                                    S_Data_reg <= {Rx_in,S_Data_reg[7:1]}; 
                                    
                           // shift in the data 
                           // First bit is LSB, so put it on the MSB side and 
                           // when next data comes it gets shifed to the LSB side
                                 end 
                              if (rst_bit_index_count==1) // bit index count reset 
                                  begin
                                      bit_index_count<=3'd0;  
                                  end
                               else 
                                    begin
                                        if(inc_BiCount==1)  // Bit index count incremented by the controller
                                           begin
                                                    bit_index_count<=bit_index_count+1'b1;
                                            end
                                        else 
                                            begin
                                                    bit_index_count<=bit_index_count;
                                             end
                                     end
                          end
                   end
